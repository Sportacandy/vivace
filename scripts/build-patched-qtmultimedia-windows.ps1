...
try {
    ...
    # QT_SYNC_HEADERS_AT_CONFIGURE_TIME=ON (above) is the real fix for the
    # "Cannot open include file: QtMultimedia/private/qtmultimedia-
    # config_p.h" failure seen without it: read qtbase's own
    # src/tools/syncqt/main.cpp -- syncqt silently skips creating a private
    # header's forwarding alias if the real (generated) file doesn't exist
    # yet at the moment it runs, and relies on a build-time dependency to
    # re-run it later once that file appears; that dependency isn't
    # reliably present for every generated config header, so without this
    # option syncqt's normal build-time ninja edge can permanently miss the
    # alias (confirmed: retrying the same build twice reproduced the exact
    # same failing files both times, ruling out a mere scheduling race).
    # This option (qtbase/cmake/QtBuildOptionsHelpers.cmake) runs syncqt
    # synchronously during cmake's own configure step instead, after the
    # config header has already been generated by qt_feature_module_end.
    #
