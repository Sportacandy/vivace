...
#                tools (qt-configure-module, private headers) present.
...
QT_CONFIGURE_MODULE="$QTDIR/bin/qt-configure-module"
if [ ! -x "$QT_CONFIGURE_MODULE" ]; then
    echo "ERROR: $QT_CONFIGURE_MODULE not found or not executable -- is \"$QTDIR\" a Qt install with qtbase dev tools?" >&2
    exit 1
fi
...
(
    cd "$WORK/build"
    "$QT_CONFIGURE_MODULE" "$WORK/qtmultimedia" \
